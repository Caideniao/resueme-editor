<template>
    <div>
        <header class="log-header"><span class="arrow" @click="goBack"></span><span>账号注册</span></header>
...
export default {
    ...
    methods:{
        ...
        goBack() {
            // this.$store.commit('setPath','/') 不需要检测到popstate变化会先调用回调函数，变store里的state.path为前页的state
            window.history.go(-1) 
        }
    }
}
...
.log-form{
    ...
    .log-button{
        ...
        background-color: rgb(33, 188, 235);
        display: block;
        margin:30px auto;
        border-radius: 6px;
